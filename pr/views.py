...
def employee(request):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK, data=msg("員工號不存在"))
...
def projectPR(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return Response(status=status.HTTP_200_OK, data=msg("無此專案或已送出考核"))
        if emp.needPR is False:
            return Response(status=status.HTTP_200_OK, data=msg("本年度不須考核"))
    ...
    if request.method == 'GET':
        ...
        if len(project) == 0:
            return Response(status=status.HTTP_200_OK, data=msg("無專案考核或尚未完成專案考核"))
...
def projectPRRetrieve(request, pk):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK, data=msg("無此專案或尚未完成專案考核"))
    if request.method == 'GET':
    ...
    if request.method == 'PATCH':
        ...
        if project.eid_id != eid:
            return Response(status=status.HTTP_200_OK, data=msg("非該考績所屬員工，無法修改"))
        serializer = ProjectPRSerializer(project,
...
def normalPR(request):
    ...
    if request.method == 'POST':
        ...
        if emp.doneNormalPR or len(nor) > 0:
            return Response(status=status.HTTP_200_OK, data=msg("已送出過四大面向考核"))
        ...
        if serializer.is_valid():
            ...
            if serializerEmp.is_valid():
                ...
                return Response(status=status.HTTP_200_OK, data=serializerEmp.errors)
            return Response(status=status.HTTP_201_CREATED, data=serializer.data)
        else:
            return Response(status=status.HTTP_200_OK, data=serializer.errors)
    else:
        if request.method == 'GET':
            ...
            if len(emp) == 0:
                return Response(status=status.HTTP_200_OK, data=msg("無四大面向考核"))
...
def normalPRRetrieve(request, pk):
    ...
    if type(eid) == Response:
        return eid
    if pk == 1231253:
        return Response(status=status.HTTP_200_OK, data=msg("testing"))
    try:
        emp = NormalPR.objects.get(eid=eid)
    except NormalPR.DoesNotExist:
        return Response(status=status.HTTP_200_OK, data=msg("尚未完成四大面向考核"))
    if request.method == 'GET':
    ...
    if request.method == 'PATCH':
        ...
        if serializer.is_valid():
            ...
            return Response(status=status.HTTP_200_OK, data=msg(serializer.data))
...
def projectReviewRecord(request, pk):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK, data=msg("無此評分紀錄"))
    if request.method == 'GET':
    ...
    if request.method == 'PATCH':
        ...
        if prRR.reviewer != eid:
            return Response(status=status.HTTP_200_OK, data=msg("非評分者無法修改"))
        serializer = ProjectReviewRecordSerializer(prRR,
...
def projectReviewRecordList(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return Response(status=status.HTTP_200_OK, data=msg("無此考績可評分"))
        if pPR.status == "pm":
            ...
            if serializer.is_valid() and serializerPPR.is_valid():
                ...
                return Response(status=status.HTTP_200_OK, data=msg("資料錯誤"))
        else:
            isSuperVisor = SupervisorInfo.objects.filter(dept_id=pPR.status, eid=eid)
            try:
                o = Order.objects.get(id=pPR.status)
            except Order.DoesNotExist:
                return Response(status=status.HTTP_200_OK, data=msg("此考績狀態有誤，請聯絡管理員"))
            if len(isSuperVisor) == 0:
                return Response(status=status.HTTP_200_OK, data=msg("非目前可評分主管"))
            chairman = ("A3", "viceCEO")
            ...
            if serializer.is_valid() and serializerPPR.is_valid():
                ...
                return Response(status=status.HTTP_200_OK, data=msg(serializer.errors + serializerPPR.errors))
    if request.method == 'GET':
...
def normalReviewRecordList(request):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK,data=msg("找不到員工"))
    if request.method == 'POST':
        ...
        if serializer.is_valid() and serializerNPR.is_valid():
            ...
            return Response(status=status.HTTP_200_OK, data=msg(serializer.errors + serializerNPR.errors))
    if request.method == 'GET':
...
def creditRecord(request):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK, data=msg("非主管"))
    if request.method == "POST":
        try:
            rec = Employee.objects.get(id=request.data["receiver"])
        except Employee.DoesNotExist:
            return Response(status=status.HTTP_200_OK,data=msg("找不到接收者"))

        if rec.creditStatus != g.dept_id:
            return Response(status=status.HTTP_200_OK, data=msg("非當前給分者"))
        ...
        try:
            ...
            return Response(status=status.HTTP_200_OK, data=msg("找不到下一階段給分者"))
        chairman = ("A3", "viceCEO")
        ...
        if serializer.is_valid() and serializerEmp.is_valid():
            ...
            return Response(status=status.HTTP_200_OK, data=msg("資料有誤"))
    if request.method == "GET":
        emp = Employee.objects.filter(creditStatus=g.dept_id)
        if len(emp) != 0:
            cre = CreditNeedRecordSerializer(emp,many=True)
        else:
            return Response(status=status.HTTP_200_OK, data=msg("無資料"))
...
def creditRecordRetrieve(request, pk):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK, data=msg("找不到此筆紀錄"))
    grade = request.data["grade"]
    credit = request.data["credit"]
    ...
    if serializer.is_valid():
        ...
        return Response(status=status.HTTP_200_OK,data=msg("更新失敗"))
...
def creditDistribution(request):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK, data=msg("當前使用者非主管職"))
    if request.method == "POST":
        recDept = request.data["receiveDept"]
        try:
            o = Order.objects.get(id=recDept, parent=isSupervisor.dept_id)
        except Order.DoesNotExist:
            return Response(status=status.HTTP_200_OK,data=msg("非可給點對象"))
...
def creditDistributionRetrieve(request, pk):
    ...
    if request.method == "PATCH":
        try:
            ...
            return Response(status=status.HTTP_200_OK, data=msg("無此紀錄"))
        creDept = request.data["creditDept"]
...
def annotation(request):
    ...
    if request.method == "GET":
        ...
        if len(anno) == 0:
            return Response(status=status.HTTP_200_OK, data=msg("無當前可查看備註"))
...
def annotationRetrieve(request, pk):
    ...
    if request.method == "PATCH":
        try:
            ...
            return Response(status=status.HTTP_200_OK, data=msg("找不到此筆資料"))
        content = request.data["content"]
    ...
    if request.method == "GET":
        try:
            ...
            return Response(status=status.HTTP_200_OK,data=msg("尚未有備註"))
...
def attendanceRecord(request):
    ...
    try:
        ...
        return Response(status=status.HTTP_200_OK, data=msg("無考勤紀錄"))
...
def CreditDept(request):
    ...
    if len(o) == 0:
        return Response(status=status.HTTP_200_OK, data=msg("無子部門"))
    else:
        deptList = list()
        for i in o:
